/**
 * Simulated Annealing (SA) Metaheuristic Engine (JavaScript)
 * 
 * Solves Traveling Salesperson Problem (TSP) using 2-opt edge swaps and
 * Metropolis-Hastings stochastic cooling schedule.
 */
...
class JSSimulatedAnnealingEngine {
    constructor(selectedCities, distMatrixFull, tInitial = 1000.0, alpha = 0.995) {
        ...
        // Build local distance matrix for selected cities
        ...
        // Matrix representation for Heatmap canvas visualization
        let maxD = 0;
        for (let i = 0; i < this.N; i++) {
            for (let j = 0; j < this.N; j++) {
        ...
        this.Q = Array.from({ length: this.N }, () => new Float64Array(this.N));
        for (let i = 0; i < this.N; i++) {
            for (let j = 0; j < this.N; j++) {
                this.Q[i][j] = (i === j) ? 0 : this.distMatrix[i][j];
            }
        }
    }
    ...
    getNeighbor2Opt(tour) {
        ...
        // 2-opt edge swap (keep start city at index 0 fixed)
    ...
    async runAnnealing(maxIter = 1000, onProgress = null) {
        ...
        for (let step = 1; step <= maxIter; step++) {
            ...
            history.push({ step, cost: currentCost, temp: T });
            T *= this.alpha;

            if (onProgress && step % updateInterval === 0) {
                onProgress(step, currentCost, T);
            }
